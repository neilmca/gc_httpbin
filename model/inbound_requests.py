...
class InboundRequest(ndb.Model):
	...
	def add_record(time_stamp, host_url, path, headers, qparams, body):
		rc = InboundRequest(time_stamp = time_stamp, request_host_url = host_url, request_path = path, request_headers = headers, request_query_params = qparams, request_body = body)
		rc.put()
	...
	def wipe_if_big():
		...
		if(len(rows) > DB_MAX_ROWS):
			...
			logging.info('wiping %d oldest records as DB has exceeded limit of %d' % (len(row_to_delete), DB_MAX_ROWS))
