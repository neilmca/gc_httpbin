...
class RootHandler(BaseHandler):
    ...
    def get(self):
        ...
        for item in data:
            ...
            resultfields = {'ts' :  item.time_stamp, 'host_url' : item.request_host_url, 'path': item.request_path, 'query_params' : item.request_query_params, 'headers': header_list, 'body': body}
    ...
    def post(self):
        ...
        for key, value in self.request.headers.iteritems():
            ...
            headers += '\r\n'
        InboundRequest.add_record(datetime.utcnow(), self.request.host_url, self.request.path, headers, self.request.query_string, self.request.body)
